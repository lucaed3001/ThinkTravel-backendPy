...
oauth2_user_scheme = OAuth2PasswordBearer(tokenUrl="auth/user/login")
router = APIRouter()
...
async def get_token_user(db: Session = Depends(get_db), token: str = Depends(oauth2_user_scheme)):
    if not token:
        raise HTTPException(status_code=400, detail="Token is missing")
    return {"token": token}

@router.get("/image", summary="Get image of a user", )
async def get_user_image_f(db: Session = Depends(get_db), token: str = Depends(oauth2_user_scheme)):
...
async def upload_image(file: UploadFile = File(...), db: Session = Depends(get_db), token: str = Depends(oauth2_user_scheme)):
    id = lib.verify_token(token)
    id = id.get("id")
    if id is None:
        raise HTTPException(status_code=400, detail="Invalid token")
...
async def get_user_data(db: Session = Depends(get_db), token: str = Depends(oauth2_user_scheme)):
    id = lib.verify_token(token)
    id = id.get("id")
    if id is None:
        raise HTTPException(status_code=400, detail="Invalid token")
